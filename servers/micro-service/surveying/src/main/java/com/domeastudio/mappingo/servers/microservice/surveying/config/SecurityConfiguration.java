    ...
    public UserDetailsService userDetailsService() {
        return new UserDetailsService() {
            ...
            public UserDetails loadUserByUsername(String name) throws UsernameNotFoundException {
                // 通过用户名获取用户信息
                TuserEntity tuserEntity = tUserService.findByNameOrEmailOrPhone(name);
                if (tuserEntity != null) {
                    // 创建spring security安全用户
                    System.out.println(tuserEntity.getRuserrolesByUid().size());
                    User user = new User(tuserEntity.getName(), tuserEntity.getPwd(),
                            AuthorityUtils.createAuthorityList(tUserService.findRoleName(tuserEntity).toArray(new String[]{})));
                    return user;
                } else {
